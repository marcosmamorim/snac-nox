...
from nox.apps.bindings_storage.pybindings_storage import Name,pybindings_storage
...
class BindingsDirectory(Component, Directory): 
  ...
  def search_id_table(self, query,name_type): 
        d = defer.Deferred() 
        def cb(row_list):
          self.process_id_table_results(row_list,d, name_type)
        
        query = query.copy()
        self.fix_query_params(query)
        mr = get_matching_rows_op("bindings_id", self.store, [query], cb) 
        
        return d

  def process_id_table_results(self, row_list, d, name_type): 
    all_ids = set() 
    for row in row_list:
      all_ids.add(row["id"])
    ...
    def cb(row_list): 
      self.process_name_table_results(row_list,d,name_type) 
    
    if len(queries) == 0:
      d.callback(queries) # no matches found 
    else: 
      mr = get_matching_rows_op("bindings_name", self.store, queries, cb) 
    
  def process_name_table_results(self,row_list,d,name_type): 
        new_list = [] 
        for row in row_list:
            if row["name_type"] == name_type:
