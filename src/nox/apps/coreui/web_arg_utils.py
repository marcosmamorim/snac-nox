...
def sort_and_filter_results(filter,res_list):
  if(len(res_list) > 1): 
...
# request's args.
# item_list : a list of string-to-string dictionaries.  
# attr_list : the list of (item-key, arg-key) pairs that indicate
#             how URL parameters from the webrequest should be used
#             to filter attributes in each item.
# args      : the actual web request's args dictionary
def filter_item_list(item_list, attr_list, args): 
  live_attrs = [] 
  regex_map = {} 
  for attr in attr_list:
    if attr in args:
        live_attrs.append(attr)
        r = re.compile(glob_to_regex(args[attr][-1]),re.IGNORECASE)
        regex_map[attr] = r
