...
steps:
    ...
    1) Resolve the webservices component
       ...
       ws = component.resolve(str(nox.apps.coreui.webservice))
...
lg = logging.getLogger('coreui.webservice')
...
def forbidden(request, errmsg):
    """Return an error code indicating client is forbidden from accessing."""
    request.setResponseCode(403)
    request.setHeader("Content-Type", "text/plain")
    request.write(errmsg.encode("utf-8"))
    request.finish()
    return server.NOT_DONE_YET

def badRequest(request, errmsg):
...
def conflictError(request, errmsg, otherURI=None):
...
def internalError(request, errmsg):
...
def methodNotAllowed(request, errmsg, valid_methods):
...
def unauthorized(request):
    ...
    request.write("You must login using 'POST /login' and pass the resulting")
    request.write(" cookie with\neach request.\n")
    request.finish()
    return server.NOT_DONE_YET
...
### Authorization
#
def authorization_failed(request, capabilities_set):
    """Verify authorization and output error message if not allowed.

    Checks for authorization (using `authui.requestIsAllowed').  If
    ...
    if not authui.requestIsAllowed(request, capabilities_set):
        session = request.getSession()
        e = """This request requires the following capabilities

    %s

You are logged in as user %s which does not have one or more of these
capabilities.""" % (", ".join(capabilities_set), session.user.username)
        forbidden(request, e)
        return True
    return False
...
class WSPathTreeNode:
    def __init__(self, parent, path_component):
        ...
        self._parent = parent
        self._children = []
        self._wsn = WhitespaceNormalizer()
    ...
    def add_child(self, handler, method, path_component, doc):
    ...
    def call_handler(self, handler):
        ...
        try :  
          ...
          lg.error("caught unhandled exception with path '%s' : %s" % \
              (str(self._request.postpath), str(e)))
...
class WSRequestHandler:
    ...
    def register(self, handler, request_method, path_components, doc=None):
        ...
        for pc in path_components:
            pn = pn.add_child(handler, request_method, pc, doc)
...
class WSRes(authui.UIResource):
    isLeaf = True
    required_capabilities = set(["defaultwebservice"])
    ...
    def __init__(self, component):
        authui.UIResource.__init__(self, component)
        self.mgr = WSRequestHandler()
        self.authui = component.resolve(authui.authui)
        self.register_request(self._get_interface_doc,
                              "GET", ( WSPathStaticString("doc"), ),
                              """Get a summary of requests supported by the
                                 v1 web service interface.""")
...
class webservice(Component):
    ...
    def install(self):
        self._versions["1"] = WSRes(self)
        self.coreui = self.resolve(str(coreui.coreui))
        for v in self._versions:
            self.coreui.install_resource("/ws.v%s" % v, self._versions[v])
