...
class FailureExceptionLogin(NOXWSLoginMgr):
    ...
    def get_credentiails(self):
        raise NOXWSError("Could not login to web service.")
...
class NOXWSResponse:
    ...
    def getExpectedBody(self, expected_status, expected_content_type):
        ...
        if self.status != expected_status:
            raise NOXWSError("Return status (%d) != expected (%d)" % (self.status, expected_status), self)
        elif self.getContentType() != expected_content_type:
            raise NOXWSError("Content-Type (%s) != expected (%s)." % (self.getContentType(), expected_content_type), self)
        return self.read()
    ...
    def _login(self):
        ...
        while retry:
            ...
            body = urllib.urlencode({ "username": user,
                                      "password": password,
                                      "last_page" : ""})
            self.httpcon.request("POST", "/login", body, headers)
...
class CmdLineParser:
    ...
    def parse(self, line):
        ...
        for c in line:
            self.c = c
            try:
                state, method = self._parser_sm[state][c]
            except KeyError, e:
                state, method = self._parser_sm[state]["DEFAULT"]
            if method != None:
                method()
        if self.arg != "":
            self.args.append(self.arg)
