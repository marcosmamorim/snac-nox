...
PyRt::PyRt(const Context* c,
    ...
    PyRun_SimpleString("import sys\n"
                       ...
                       "from nox.apps.pyrt import init_python\n"
                       "init_python()\n"
                       );       
    if (PyErr_Occurred()) {
...
PyRt::initialize_oxide_reactor()
{
    ...
    PyObject* m = PyImport_ImportModule("nox.apps.pyrt.pyoxidereactor");

    if (!m) {
        throw runtime_error("Could not import pyvigilreactor from "
                            "nox.apps.pyrt.pyoxidereactor:\n" +
                            pretty_print_python_exception());
    }
    PyObject* d = PyModule_GetDict(m);
    // d is borrowed from GetDict, don't DECREF
...
get_pyevent_ctor()
{
    PyObject* pname = PyString_FromString("nox.apps.pyrt.pycomponent");
    if (!pname) {
