                             ...
                             bool check_nat)
{
    ...
    Flow_in_event::DestinationList::reverse_iterator iter = flow_in.destinations.rbegin();
    for (; iter != flow_in.destinations.rend(); ++iter) {
        if (!iter->allowed) {
            iter->allowed = true;
            break;
        }
    ...
    if (iter != flow_in.destinations.rend()) {
        iter->allowed = false;
    }
}
