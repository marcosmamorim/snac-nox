...
from nox.apps.storage.storage import StorageException, Storage

pyunit = __import__('unittest')

log = logging.getLogger('storage test')

class StorageTestBase(unittest.NoxTestCase):     
    ...
    def testAddthenPutthenGet(self):
        ...
        def get_ok(res):
            ...
            self.failUnless(row['COLUMN_INT'] == 1234)
            self.failUnless(row['COLUMN_STR'] == u'string123\u1234foo')
            self.failUnless(row['COLUMN_FLOAT'] == 3456.) 
    ...
    def testAddthenPutthenGetNextUsingIndex(self):
        ...
        def get_next(res):
            ...
            if result[0] == Storage.NO_MORE_ROWS:
                ...
                self.failUnless(self.rows <= 2)
                self.failUnless(row['COLUMN_INT'] == 1234)
                self.failUnless(row['COLUMN_STR'] == 'string123')
                self.failUnless(row['COLUMN_FLOAT'] == 3456.0 or 
    ...
    def testAddthenPutthenGetDoubleIndexCheck(self):
        ...
        def get_next2(res):
            result, ctxt, row = res
            self.failUnless(result[0] == Storage.SUCCESS)
            self.failUnless(row['COLUMN_INT2'] == 1234)
            self.failUnless(row['COLUMN_STR'] == 'string123')
            self.failUnless(row['COLUMN_INT1'] == 7890)
            self.failUnless(row['COLUMN_INT3'] == 1234)
        ...
        def get_next1(res):
            ...
            self.failUnless(result[0] == Storage.SUCCESS)
            self.failUnless(row['COLUMN_INT1'] == 1234)
            self.failUnless(row['COLUMN_STR'] == 'string123')
            self.failUnless(row['COLUMN_INT2'] == 3456)
            self.failUnless(row['COLUMN_INT3'] == 1234)
    ...
    def testMultiIndex(self):
        ...
        def remove(res):
            ...
            self.failUnless(result[0] == Storage.SUCCESS)
            self.failUnless(row['DPID'] == 6)
            self.failUnless(row['PORT'] == 7)
            self.failUnless(row['DL_ADDR'] == 8)
            self.failUnless(row['NW_ADDR'] == 200)
            self.failUnless(row['ID'] == 5)
    ...
    def testMultiIndexModify(self):
        ...
        def modify2(res):
            ...
            self.failUnless(row['PORT'] == 7)
            self.failUnless(row['DL_ADDR'] == 8)
            self.failUnless(row['NW_ADDR'] == 201)
            self.failUnless(row['ID'] == 5)
