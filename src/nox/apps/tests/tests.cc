...
public:
    ...
    void testPython() {
        ...
        n = PyString_FromString("nox.apps.tests.pytests");
        m = PyImport_Import(n);
        if (PyErr_Occurred()) {
            PyErr_Print();
            PyErr_Clear();
            BOOST_FAIL("Unable to import nox.apps.tests.pytests.");
        }        
...
private:
    ...
    vector<test_case*> gather_python_tests(list<string> args) {
        ...
        m = PyImport_ImportModule("nox.apps.tests.unittest");
        if (!m) {
            PyErr_Print();
            PyErr_Clear();
            throw runtime_error("Unable to import nox.apps.tests.unittest.");
        }        
