  ...
  switch(info->cur_state) {
    ...
    case READ_SRC_NAMES:
      if(info->src_key_query.size() > 0) { 
        b_store->get_names(info->src_key_query, Name::LOC_TUPLE, 
          boost::bind(&User_Event_Log::get_names_cb,this,_1,info)); 
        return;
      } 
    ...
    case READ_DST_NAMES:
      if(info->dst_key_query.size() > 0) { 
        b_store->get_names(info->dst_key_query, Name::LOC_TUPLE, 
          boost::bind(&User_Event_Log::get_names_cb,this,_1,info)); 
        return; 
      }
...
void User_Event_Log::read_main_cb(const Result & result, const Context & ctx, 
                                const Row &row, GetEntryInfo_ptr info){ 
...
void User_Event_Log::read_names_cb(const Result & result, const Context & ctx, 
                                const Row &row, GetEntryInfo_ptr info){ 
...
// or an error occurs
void User_Event_Log::get_logids_cb(const Result & result, const Context & ctx, 
                                const Row &row, GetLogidsInfo_ptr info){ 
    if(result.code == storage::Result::NO_MORE_ROWS) { 
