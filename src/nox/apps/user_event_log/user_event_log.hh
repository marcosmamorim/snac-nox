...
private:
    ...
    void read_main_cb(const Result & result, const Context & ctx, 
                                const Row &row, GetEntryInfo_ptr info);
    void read_names_cb(const Result & result, const Context & ctx, 
                                const Row &row, GetEntryInfo_ptr info);
    void finish_get_entry(GetEntryInfo_ptr info);
   
    // helper functions for getting logids associated with a name
    void get_logids_cb(const Result & result, const Context & ctx, 
                            const Row &row, GetLogidsInfo_ptr info); 

    // helpers related to clearing all uel state
    void remove_cb1(const Result &r,Clear_log_callback cb, int64_t logid); 
    void remove_cb2(const Result &r, Clear_log_callback cb); 
